...
class OrderSupportServiceProvider extends ServiceProvider
{
    ...
    public function handleProcessOrder(Collection $products, string $token, array $sessionData, Request $request)
    {
        ...
        $addressData = OrderHelper::processAddressOrder($currentUserId, $sessionData, $request);
        ...
        $sessionData = array_merge($sessionData, ['marketplace' => $mpSessionData], $addressData);
    ...
    protected function cartGroupByStore(Collection $products)
    {
        ...
        foreach ($products as $product) {
            $store_id = $product->original_product && $product->original_product->store_id ? $product->original_product->store_id : 0;
    ...
    public function setEmailVariables($orders)
    {
        ...
        return EmailHandler::setModule(ECOMMERCE_MODULE_SCREEN_NAME)
            ->setVariableValues([
                ...
                'product_list'     => view('plugins/marketplace::emails.partials.order-detail',
    ...
    public function afterOrderStatusCompleted(OrderModel $order)
    {
        ...
        if ($order->store && $order->store->customer) {
            ...
            if ($vendorInfo->id) {
                ...
                $feePercentage = get_marketplace_setting('fee_per_order', 0);
